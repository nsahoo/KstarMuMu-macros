...
//calculate reco eff
void calR_EffInCosK(TString data, TString type, int iBin){

  int nrec(0), nrecpass(0);
  const int nbins = 6;
  double ckRangeDn[nbins]={-1., -0.7, 0., 0.4, 0.8, -1.};
  double ckRangeUp[nbins]={-0.7, 0., 0.4, 0.8, 1., 1.};
  ...
  for (int entry = 0; entry < ch->GetEntries(); entry++){
    ...
    if (gCosK > ckRangeUp[iBin] || gCosK < ckRangeDn[iBin]) continue;
    ...
    if(type == "gen"){
      ...
      if (CosK > ckRangeUp[iBin] || CosK < ckRangeDn[iBin]) continue;
    }
  ...
  printf("\n");
  printf("INFO: gen cosk range: [ %.2f-%.2f ] \n", ckRangeDn[iBin], ckRangeUp[iBin]);
  ...
  int main(int argc, char** argv) {
	...
	calR_EffInCosK(data,type,iBin);
      }
    }else{
      printf("Wrong argument given, please xcheck !\n");
      printf("./calR_EffInCosK data type\n");
